...
class CommentController extends Controller {
	public function get( $post_id ) {
		...
		foreach( $comments as $comment ) {
			...
			$comment->reactions = $req
			// ->addSelect( 'images.image_url AS photo_url' )
								->get();
		}
		$comments = UserController::setCuratedFields($comments);
	...
	public function post( Request $request, $post_id ) {
		...
		$author_id = Post::findOrFail($post_id)->author_id;
		$user = User::findOrFail($author_id);
		
		$user->notify( new PostComment( Auth::user(), $comment ) );
