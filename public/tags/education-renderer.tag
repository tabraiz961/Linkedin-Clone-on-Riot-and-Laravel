<education-renderer>
		...
		<div class="card-section pt-0">
			<div class="paragraph" each={ item in items }>
				...
				<div>
					...
					<div class="profile-titles-container job-multiple-expreiences2">
			...
			<div class="paragraph" if={ items.length == 0 }>
				<p class="text-center">Insert your Education to get connected with colleagues .</p>
			</div>
		</div>
	</div>
	...
	<div ref="removeInstituteModal" class=" tiny reveal" id="removeInstituteModal" data-reveal data-close-on-click="true">
		...
		<div class="removeInstituteBtns" >
			<button onclick={removeInstitute} class="button removeInstitute" style="background: red;" if={ removalInstitute }>Remove { removalInstitute.institute.institution_name }</button>
	...
	<div ref="removeExperienceModal" class=" tiny reveal" id="removeExperienceModal" data-reveal data-close-on-click="true">
		...
		<div class="removeInstituteBtns" >
			<button onclick={ removeExperience } class="button removeInstitute" style="background: red;" if={ removalExperience }>Remove experience of { removalExperience.experience_title }</button>
	...
	<script>
		...
		removeInstitute(){
			...
			window.axios.post( opts.basepath + '/user/experience/institute/remove/'+that.removalInstitute.institute.institution_id, { 'experience_type_id': 2 } )
		...
		removeExperience(){
			...
			window.axios.post( opts.basepath + '/user/experience/job/remove/'+that.removalExperience.experience_id, {  'experience_type_id': 2 } )
			.then(function (response) {
				if (response.data && response.data.success) {
		...
		generate(e) {
			...
			if(that.edit_experience_id){
				...
				obj['experience_type_id'] = 2;//job experience 
				obj['address_id'] = 0;
			}
			return obj;
		}
		...
		promotionLinesCreator() {
			var _continer = document.getElementsByClassName('job-multiple-expreiences2');
